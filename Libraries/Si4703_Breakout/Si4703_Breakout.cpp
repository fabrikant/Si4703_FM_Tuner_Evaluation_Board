...
///////////////////////////////////////////////////////////////////////////
void Si4703_Breakout::updateInfo(){

	uint16_t temp;
	infoFreq = getChannel();
	
	temp = si4703_registers[STATUSRSSI] >> 8;
	infoStereo = temp & 0x0001;
	
	infoSignalLevel = si4703_registers[STATUSRSSI] & 0x007F;
	infoVolume = si4703_registers[SYSCONFIG2] & 0x000F;
	
	temp = si4703_registers[POWERCFG] >> DMUTE;
	infoMute = (temp & 0x0001)? false:true;
	
	temp = si4703_registers[POWERCFG] >> SMUTE;
	infoSoftMute = (temp & 0x0001)? false:true;
	
	boolean enable = si4703_registers[POWERCFG] & 0x0001;
	temp = si4703_registers[POWERCFG] >> 6;
	boolean disable = temp & 0x0001;
	
	infoStandBy = (!enable || disable) ? true:false;
	
}

void Si4703_Breakout::mute(){
//стоит 11
//надо  10
	updateInfo();
	si4703_registers[POWERCFG] = si4703_registers[POWERCFG] ^ (B01 << DMUTE);
	updateRegisters();
}

void Si4703_Breakout::unMute(){
	updateInfo();
	si4703_registers[POWERCFG] =  si4703_registers[POWERCFG] | (B11 << DMUTE);
	updateRegisters();
}

void Si4703_Breakout::muteSwitch(){
...
void Si4703_Breakout::standBy(){
	updateInfo();
	si4703_registers[POWERCFG] = si4703_registers[POWERCFG] >> 7;
	si4703_registers[POWERCFG] = si4703_registers[POWERCFG] << 7;
	si4703_registers[POWERCFG] += 0x0041; 
	updateRegisters();
}

void Si4703_Breakout::standUp(){

	updateInfo();
	si4703_registers[POWERCFG] = si4703_registers[POWERCFG] >> 7;
	si4703_registers[POWERCFG] = si4703_registers[POWERCFG] << 7;
	si4703_registers[POWERCFG] += 0x0001; 
	updateRegisters();
}

void Si4703_Breakout::standSwitch(){
...
void Si4703_Breakout::si4703_init() 
{
	...
	si4703_registers[SYSCONFIG3] = 0x0007; //Set sensitive
	updateRegisters(); //Update
...
int Si4703_Breakout::getChannel() {
	...
	uint16_t temp;
	temp = si4703_registers[SYSCONFIG2]>>6;
	temp = temp & 0x003;
	
	
	if (temp == 0) {beginBand = 8750;}
	
	temp = si4703_registers[SYSCONFIG2]>>4;
	temp = temp & 0x003;
	if (temp==0) {
		space = 20;
	}else if (temp==1){
	...
	channel = (space*channel+beginBand)/10;
	return(channel);
}
